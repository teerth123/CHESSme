...
export default function Slate({ username }) {
    ...
    useEffect(() => {
        if (username) {
            axios.post("http://localhost:3000/api/v1/info", { username })
                .then(response => {
        ...
        >
            ...
            {hoverDev && (
                <div className="absolute left-0 mt-1 p-1 w-min text-white rounded bg-[#363433]">
                    <div>A: <a href="https://example.com" className="text-blue-400" target="_blank" rel="noopener noreferrer">example.com</a></div>
                    <div>T: <a href="https://example.com" className="text-blue-400" target="_blank" rel="noopener noreferrer">example.com</a></div>
                </div>
            )}
        </div>
    </div>
